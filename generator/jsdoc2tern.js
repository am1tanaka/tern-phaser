      ...
      if (parent) {
        ...
        switch(jsdocItem.kind) {
          case "member":
            ...
            // change * in Generic Object type to any.
            if (type) {
                // Change enum type
                if (jsdocItem.isEnum) {
                    type = "+Enum"+type.charAt(0).toUpperCase()+type.slice(1);
                }
            }
            break;
          case "function":
            ...
            let str = type;
            let debug = 0;
            let strmatch = /^fn\(.*?:.*?\+Object\.<.*?\*.*?>.*?\)/;
            while (arg=str.match(strmatch)) {
                let any = arg[0].replace(/\*/g, 'any');
                str = str.replace(strmatch, any);
                debug++;
                if (debug > 10) break;
            }
            type = str;
